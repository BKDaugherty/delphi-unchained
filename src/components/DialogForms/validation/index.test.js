import {validEthAddress, validInteger, validPositive, validRequired, validTimeInTheFuture} from './index'
...
const GenSuccessTest = ({ validator, data }) => {
    return test(
        `${validator.name} Validator returns no error with correct input`,
...
const GenFailTest = ({ validator, data }) => {
    return test(
        `${validator.name} Validator returns error : ${validator.error}`,
...
const SuccessTests = [
    ...
    CreateTest(
        ...
        '0x9c380f3ce900cef27640bdff53661bfa10817c9c'
    )
].map(GenSuccessTest)
