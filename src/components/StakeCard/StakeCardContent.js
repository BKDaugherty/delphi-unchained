    ...
    return (
        <Grid container>
            <Grid direction="row" spacing={24} container>
                ...
                <Grid item>
                    <EthAddressDisplayCard
                        title={'Amount'}
                        subheader={`${stake.claimable_stake} ${stake.token.symbol} ${
                            stake.token.name
                        }`}
            ...
            Claims
                    ...
                    {claims_list.length > 0
                        ? claims_list.map(claim => (
                            <Grid item key={`claim-key-${claim.id}`}>
            ...
            Whitelisted Claimants
                    ...
                    {whitelist.length > 0
                        ? whitelist.map(addr => (
                            <Grid item key={`whitelisted-claimant-${addr.claimant}`}>
                                <EthAvatarIcon address={addr.claimant} />
                            </Grid>
                        ))
                        : null}
                </Grid>
