...
export const proposeSettlement = (ethAddress, stakeAddress) => async ({
    claimId,
    amount
}) => {
    const stake = await DelphiStake.at(stakeAddress)
...
export const acceptSettlement = (ethAddress, stakeAddress) => async ({
    claimId,
    settlementId
}) => {
    const stake = await DelphiStake.at(stakeAddress)
...
export const settlementFailed = (ethAddress, stakeAddress) => async ({
    claimId
}) => {
    const stake = await DelphiStake.at(stakeAddress)
...
export const ruleOnClaim = (ethAddress, stakeAddress) => async ({
    claimId,
    ruling
}) => {
    const stake = await DelphiStake.at(stakeAddress)
