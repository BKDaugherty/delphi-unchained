# Delphi Unchained
## Got the Stake Stuff started!!!
### Steps to reproduce (All should be done in docker container deth)
* Initialize deth and ganache `$ docker-compose up --build`
* clone this repo into your deth environment
* in the cloned repo run `$ npm install` --> Ensures that you get the correct build of scrypt
* then run `$ truffle install` --> Should build the dependency contracts and create the 'installed_contracts' directory
* next run `$ yarn run dev` --> Runs a quick lil script to deploy the contracts to the network, and copy their artifact histories
  into the src/artifacts file so that webpack can see them. Then starts the development server
* Ensure that metamask is installed in your browser
* Open Metamask and in the top left set it to private network (port 8545)
* Reload a metamask account from the seed phrase outputted by the ganache container
* Press 'Make your stake'
* In the dialog box opened by metamask press submit!
* Check the dev tools console to see both the Stake object as well as a called value!
* Next up is to figure out how to use the initialize function!  



 --------------------------- PAST HERE IS GENERATED BY CREATE REACT APP FOR QUICK LOOKS ---------------------------
